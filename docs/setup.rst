...
Note: If the callsign-box does not turn green there may be two reasons: 1: The API key entered is wrong (then it normally turns red instead) or 2: The self-signed SSL-certificate is not known to the browser (then try connecting the tracker directly from the browser and accept the certificate).
