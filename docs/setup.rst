...
The first time you are opening the webpage on the tracker this way, the browser will complain that its certificate cannot be verified. This is because it is a self-signed certificate and not signed by a known CA. If you feel confident that you are really connecting to your tracker you can add an exception to the browser to let it accept the webpage. Later connections to the same tracker will then work, even if the webapp is coming from somewhere else. From version 4.1. the certificate is generated by the tracker itself when callsign is set or by using the command *newcert*. The certificate's common-name will be based on the ESP32 chip's unique id, but information about the callsign will be provided as well.
