...
Encryption is based on AES-256 which is strong encryption scheme. To be more specific, it is AES-GCM-SIV which also authenticates the packets. It is *symmetric* crypto which means that all participants must have the same key to communicate. This scheme is supported by Polaric Server and is documented here. It is open-source (free software). Packets encrypted by Arctic Tracker may be decrypted by Polaric Server as long as the key is the same in both ends. 
